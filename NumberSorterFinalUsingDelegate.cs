...
class Program
{
    ...
    //function to recursivly sort the array 
    static void SortRecursivly(ref double[] dArray, getArrayValueDelegate getAVD)
    {
        Console.WriteLine("dArray unsorted", dArray); 
        //declare the value used in the recursive function
        double value = null; 

        //base case
        //if array.length <= 1 print the array
        //if there is 1 or less values in the array, the array cannot be sorted 
        if((dArray <= 1)
        {
        ...
        {
            //get the value used, using the getValueDelegate method
            value = getAVD(dArray); 
            //use the RemoveArrayValue function to remove the value from the new array 
            RemoveArrayValue(ref dArray, value);
            //RECURSION
            //call the function to sort 
            SortRecursivly(dArray, getAVD);
        }
        //print sorted array
        Console.WriteLine("dArray sorted", dArray);
    }
    ...
    start:
        ...
        //variables were called at the top of Main
        SortRecursivly(aUnsorted, getArrayValue); 
    }
}